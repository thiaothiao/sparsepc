...
namespace sparsepc
{
    ...
    {// TODO optimize memory usage
        ...
        while (std::getline(matrixDataFile, matrixRowString)) // here we read a row by row of matrixDataFile and store every line into the string variable matrixRowString
        {
            ...
            while (std::getline(matrixRowStringStream, matrixEntry, sep)) // here we read pieces of the stream matrixRowStringStream until every comma, and store the resulting character into the matrixEntry
            {
                matrixEntries.push_back(static_cast<Scalar>(std::stod(matrixEntry)));   //here we convert the string to double and fill in the row vector storing all the matrix entries
            }
        ...
        const RMMatrix<ScalarType> matrix = Eigen::Map<RMMatrix<Scalar>>(matrixEntries.data(), matrixRowNumber, matrixEntries.size() / matrixRowNumber);
        return matrix;
    }
}
