...
namespace sparsepc
{
    namespace linearmodel
    {
        ...
        auto SparsePC<ModelImplementationType>::run(const Matrix<Scalar>& sigma) const
        {
            ...
            sparseSolutions.push_back(ModelImplementationType{ m_Param.modelParams[0] }.run(sigma));
            ...
            for(Index j = 1; j < m_Param.nbComponents; ++j)
            {
                ...
                sparseSolutions.push_back(ModelImplementationType{ m_Param.modelParams[j] }.run(inverse * B * sigma * B));

                q = B * sparseSolutions.back().vector;

                sparseSolutions.back().value = (q.transpose() * sigma * q).value() / q.squaredNorm();
            }            
            ...
            ProgressBar* progressBar)
        {
            ...
            else
            {
                ...
                for (auto& cpnt : candidates)
                {
                    ...
                    cpnt.value =
                        (cpnt.q.transpose() * sigma * cpnt.q).value() / cpnt.q.squaredNorm();
                }
